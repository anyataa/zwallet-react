    ...
    <div className="right">
      ...
      >
        <p className="transfer-primary-text" style={{ margin: "30px 0" }}>
          Search Receiver By Phone Number
        </p>
        <Link to="/transfer" style={{ textDecoration: "none" }}>
          <Button>Back to Friend List</Button>
        </Link>
      </div>
      <div className="transfer-input-amount-wrapper2">
        ...
        {Object.keys(data).length > 0 ? (
          <div className="transfer-item-wrapper" style={{ width: "100%" }}>
            ...
            >
              ...
              <RiSendPlane2Fill
                onClick={() =>
                  window.location.replace(`/transfer/${data.userId}`)
                }
                size="30"
                color="#6379F4"
                className="transfer-icon-btn"
        ...
        {errorMsg ? (
          <p className="text-validation" style={{ marginTop: "50px" }}>
            {errorMsg}
          </p>
        ) : null}
      </div>
